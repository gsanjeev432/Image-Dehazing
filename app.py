...
net = tf.keras.models.load_model('trained_model', compile=False)
...
with col1:
    algo = st.selectbox("Select Algo", ['Guided', 'DarkChannel', 'GMAN'])
...
with col2:
    ...
    if upload_file_1 is not None:
        ...
        with col2:
            ...
            if algo == 'GMAN':
                recovered_image, mse_noise, psnr, ssim_noise = GMAN(image_1, image_2)
                st.image(recovered_image, width=300)
...
with col3:
    ...
    with container:
        ...
        if algo == 'GMAN' and upload_file_1 and upload_file_2 is not None:
            recovered_image, mse_noise, psnr, ssim_noise = GMAN(image_1, image_2)
            st.markdown(
                '<div class="metric-row"><span class="metric-label">MSE:</span><span class="metric-value">{}</span></div>'.format(mse_noise), unsafe_allow_html=True)
            st.markdown(
